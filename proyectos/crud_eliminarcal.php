...
if (isset($_POST['eliminar'])) 
  $id_calificacion = mysqli_real_escape_string($conexion, $_POST['id_calificacion']);
  
  // Primero, verificar si el registro existe
  $sql_verificar = "SELECT * FROM calificacion WHERE id_calificacion= '$id_calificacion'";
  $resultado_verificar = mysqli_query($conexion, $sql_verificar);
  
  if (mysqli_num_rows($resultado_verificar) > 0) {
    // El registro existe, proceder a eliminar
    $sql_eliminar = "DELETE FROM calificacion WHERE id_calificacion = '$id_calificacion'";
