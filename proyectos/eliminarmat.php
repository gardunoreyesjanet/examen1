...
<fieldset>
  ...
  <form method="post" action="crud_eliminarmat.php">
    ...
    <select name="id_materia" required>
      ...
      <?php
      $sql = "SELECT id_materia, nombre_materia,descripcion FROM materia ORDER BY id_materia";
      $resultado = mysqli_query($conexionmat, $sql);
      
      while ($fila = mysqli_fetch_assoc($resultado)) {
        echo "<option value='{$fila['id_materia']}'>id_materia: {$fila['id_materia']} - {$fila['nombre_materia']} ({$fila['descripcion']})</option>";
      }
