...
if (isset($_POST['eliminar'])) 
  $clave_docente = mysqli_real_escape_string($conexion, $_POST['clave_docente']);
  
  // Primero, verificar si el registro existe
  $sql_verificar = "SELECT * FROM docentes WHERE clave_docente = '$clave_docente'";
  $resultado_verificar = mysqli_query($conexion, $sql_verificar);
  
  if (mysqli_num_rows($resultado_verificar) > 0) {
    // El registro existe, proceder a eliminar
    $sql_eliminar = "DELETE FROM docentes WHERE clave_docente = '$clave_docente'";
    ...
    echo "<script>
            alert('❌ No se encontró ningún docente con ese ID');
            window.location.href = '../eliminarmat.php';
          </script>";
  }